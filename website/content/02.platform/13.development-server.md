...
Proxmox itself is installed by hand. From there the host is a terraform root,
`deployment/terraform/proxmox`, using `bpg/proxmox`: the Ubuntu cloud images, the two VMs with
their passthrough disks, and the cloud-init files that lay down the compose stack on one and
run the node kit on the other. Everything cloud-init writes comes from this repository, and
nothing secret passes through terraform; its README is the runbook.
...
deployment/docker-compose/
  compose.dev-server.yml                    # includes each piece below with its override
  compose.elasticsearch-dev-server.yml      # es-1, es-2, es-3 — one service per passthrough disk
  compose-otel.dev-server.yml               # Grafana asks for a login
  compose.kinotic-migration.dev-server.yml  # production profile, cluster credentials
  compose.kinotic-server.dev-server.yml     # LAN ports, env files, certificate and key mounts
  dev-server.env.example                    # every variable the two env files carry
...
/etc/kinotic/                               # on the VM, outside the repo
  dev-server.env                            # written by terraform: hostnames, endpoints, AZURE_CLIENT_ID
  secrets.env                               # placed by hand: AZURE_CLIENT_SECRET, passwords, master key, GitHub App
  platform-secrets/jwt-signing-keys         # placed by hand
...
| `kinotic.domain.secretStorage.masterKey` | from the secrets file | `SecretNameDeriver` derives every stored secret's name from it, so it is generated once and carried to the cloud |
...
| `kinotic.managementApi.github.appPrivateKey` / `webhookSecret` | from the env file | The cloud's App; its webhook URL points at `https://dev.kinotic.ai:58503/api/github/webhook` |
| `AZURE_CLIENT_ID` / `AZURE_CLIENT_SECRET` / `AZURE_TENANT_ID` | from the env file | The `dev-server` root's service principal |

The Loki, Tempo, and Mimir URLs the server queries are the ones `compose.kinotic-server.yml`
already sets, and `compose-otel.yml` already publishes Loki's 3100, the collector's OTLP 4318,
and Mimir's 9009 on the VM's interfaces, which is where the node's Alloy ships to.
...
proxmox root writes to `/etc/kinotic/dev-server.env`; its `secrets_env` output is the
principal's secret, which the operator places in `/etc/kinotic/secrets.env` by hand.
...
2. **Host.** Proxmox installed by hand on disk 0; `deployment/terraform/proxmox` creates both
   VMs, and cloud-init lays down the stack on one and the node kit on the other.
3. **Platform VM.** `generate-secrets.sh`, the Azure secret and the GitHub App's key filled
   in, `sync-platform.sh secrets`; certbot; start `kinotic-dev-server`, which runs the
   migration and the server; register the snapshot repository and SLM policy; confirm
   sign-up mail arrives and the portal loads on `https://dev.kinotic.ai`.
4. **Node VM.** Create the SYSTEM machine in the system console, place its credentials in
...
- A first system user. The migration runs with the `production` profile, so no fixture user
  ...
  in step 4, needs a bootstrap for a first system user.
