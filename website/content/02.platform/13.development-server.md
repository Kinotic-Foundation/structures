...
| Drive | Holds |
|---|---|
| 1 | Proxmox, every container's root filesystem, the node VM's OS disk and its data disk (`/var/lib/docker` and `/var/lib/kinotic/workloads`, XFS `prjquota`), the Loki/Tempo/Mimir/Grafana data under `/var/lib/kinotic/data` |
...
allocation watermark, and the Proxmox drive's largest tenants are the node VM's data disk and
the stores' retention.
...
Ubuntu 22.04 (what the kit is verified on), `cpu: host`, a second virtual disk on `local-zfs` for
the workloads. cloud-init splits it into two XFS partitions mounted with `prjquota` before the
kit runs:
