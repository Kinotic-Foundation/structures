...
| Drive | Holds |
|---|---|
| 1 | Proxmox, every container's root filesystem, the Loki/Tempo/Mimir/Grafana data under `/var/lib/kinotic/data` |
...
| Proxmox | — | — | the rest, ~70 GB | ZFS ARC, and headroom to grow the ES heaps |
...
| `kinotic.systemApi.deployment.serverHost` / `serverPort` / `serverUseSsl` | the server's LAN IPv4, `58503`, `true` | Workloads on the nodes reach the gateway across the LAN; a `CLOUD_HYPERVISOR` node rejects a hostname here. The guest trusts the Let's Encrypt chain |
...
| `KINOTIC_NODE_ID` | `dev-node-1`, `dev-node-2` |
...
| Nodes | none | Ubuntu and the kit again; the checkouts are redeployable |
