...
`SiteStorageService` is the account's data plane: an upload URL whose user delegation SAS is
scoped to the one site's directory (`sr=d`, signed through the Data Lake SDK), so the publish
workload holding it can write nowhere else in the shared account; the cleanup of other
commits' files; and a site's removal. `dist` is uploaded as it is, so a build needs no base
path; every blob carries metadata `commit=<sha>`, and the finalize step deletes every blob of
the site stamped with a commit other than the current one.
...
- **The publish task.** The deploy job's fifth task, **Publish UIs**, mints a label with a
  ...
  account's hostname as its only egress, then finalizes: waits for new sites to serve,
  adopts returning ones, orphans vanished ones, keeps the files of the current commit and
  deletes the rest. While the site provisioner is disabled nothing serves, so nothing is
  uploaded. The exit check is shared with the sync task, and the previous publish workload
  is retired by the next run's target resolution.
