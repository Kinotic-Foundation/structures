...
For each manifest:
  ...
  env          the entrypoint's environment, as raw `lxc.environment:` lines in
               /etc/pve/lxc/<vmid>.conf, which the API does not take for these containers yet
               (bpg/terraform-provider-proxmox#2789). A secrets env file the operator placed
               on the host is merged in, so no secret passes through terraform.
...
(the migration) runs to completion and `verify` once per configuration, and is not kept alive.
  ...
  kinotic-apply-container.py <manifest.json>...
"""
import hashlib
import json
import os
...
def raw_lines(env, console_log):
    """The lines this script owns in the config: the environment, and the console log."""
    lines = [f"lxc.environment: {key}={value}" for key, value in env.items()]
    if console_log:
        lines += [f"lxc.console.logfile: {console_log}",
                  f"lxc.console.size: {CONSOLE_LOG_SIZE}",
                  "lxc.console.rotate: 1"]
    return sorted(lines)


def owned(line, env, console_log):
    key, _, value = line.partition(":")
    key, value = key.strip(), value.strip()
    if key == "lxc.environment":
        return value.split("=", 1)[0] in env
    return bool(console_log) and key.startswith("lxc.console.")


def config_in_sync(vmid, env, console_log):
    with open(conf_path(vmid)) as f:
        lines = f.read().splitlines()
    return sorted(l for l in lines if owned(l, env, console_log)) == raw_lines(env, console_log)


def write_config(vmid, env, console_log):
    """Replaces the owned lines and leaves every other line alone."""
    with open(conf_path(vmid)) as f:
        lines = f.read().splitlines()
    kept = [l for l in lines if not owned(l, env, console_log)]
    with open(conf_path(vmid), "w") as f:
        f.write("\n".join(kept + raw_lines(env, console_log)) + "\n")
...
def apply(manifest):
    ...
    in_sync = (config_in_sync(vmid, env, console_log)
    ...
    if status(vmid) == "running":
        pct("stop", str(vmid))
    write_config(vmid, env, console_log)
    place(files, dirs)
    if console_log:
