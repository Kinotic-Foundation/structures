...
export default class NamespaceList extends Vue {
  ...
  async toApplicationPage(item: Identifiable<string>): Promise<void> {
    try {
      const app = await this.dataSource.findById(item.id)
      APPLICATION_STATE.setCurrentApplication(app)
      this.$router.push(`/application/${encodeURIComponent(item.id)}`)
    } catch (e) {
      console.error('[NamespaceList] Failed to set current application:', e)
    }
  }
...
<template>
  <ContainerMedium>
    ...
    <CrudTable createNewButtonText="New application" rowHoverColor="" :data-source="dataSource" :headers="headers"
      :singleExpand="false" @add-item="onAddItem" @edit-item="onEditItem" ref="crudTable"
      @onRowClick="toApplicationPage" :enableViewSwitcher="true">
      <template #item.id="{ item }">
        <span>{{ item.id }}</span>
      </template>
      <template #additional-actions="{ item }">
        <Button v-if="item.enableGraphQL" text class="!text-[#334155] !bg-white" :title="'GraphQL'"
          @click="openGraphQL">
          <img src="@/assets/graphql.svg" />
        </Button>

        <Button v-if="item.enableOpenAPI" text class="!text-[#334155] !bg-white" :title="'OpenAPI'">
