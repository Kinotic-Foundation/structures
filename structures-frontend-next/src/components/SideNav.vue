                    ...
                    <template v-for="(subItem, index) of item.children" :key="index">
                        <div class="cursor-pointer relative px-3.5 py-2 flex items-center transition-all" @click="navClicked(subItem)">
                            <svg width="18" height="44" viewBox="0 0 18 44" fill="none" xmlns="http://www.w3.org/2000/svg" class="absolute left-4 -top-2">
                                <path :d="getLinePath(index, item.children.length)" class="stroke-surface-600" stroke-width="2" />
                                <path
                            ...
                            <span
                                ...
                                :class="selectedNav?.label === subItem.label ? 'text-surface-0' : 'text-surface-400 hover:text-surface-0'"
                            >{{ subItem.label }}</span
                            >
                        </div>
                    </template>
